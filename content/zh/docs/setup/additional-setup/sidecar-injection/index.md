...
在部署应用程序时，可以考虑创建辅助 pod 作为中间步骤。例如 [OpenShift Source to Image Builds](https://docs.okd.io/latest/builds/understanding-image-builds.html#build-strategy-s2i_understanding-image-builds)。创建这样的辅助 pod 可以用于将应用程序的源码编译成二进制包。编译二进制完成后，应用程序的 pod 就可以运行了。而用于编译的辅助 pod 则被丢弃。因此这些中间辅助的 pod 不应该有 Istio sidecar。即使在策略被设置为 `enabled`，并且名称空间被正确标记为自动注入的情况下，这些中间辅助的 pod 也不应该有 Istio sidecar。
